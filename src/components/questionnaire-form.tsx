...
export function QuestionnaireForm({
    ...
    return (
      <div className="guest-results">
        <div className="guest-mode-bar" role="note">
          <div className="guest-mode-copy">
            <span className="guest-mode-icon"><DatabaseZap size={18} /></span>
            <div><strong>Practice plan — not saved</strong><p>This plan only exists on this page. Refreshing or leaving clears your numbers.</p></div>
          </div>
          <div className="guest-mode-actions">
            <button className="button ghost compact" type="button" onClick={() => setShowGuestPlan(false)}><RotateCcw size={16} /> Edit numbers</button>
            <Link className="button primary compact" href="/auth">Create account</Link>
          </div>
        </div>
  ...
  return (
    <form className="form-card planner-form" onSubmit={submit}>
      {guestMode && (
        <div className="guest-warning" role="note">
          <span className="guest-warning-icon"><LockKeyhole size={18} /></span>
          <div><strong>You’re planning as a guest</strong><p>Nothing you enter will be saved to an account or database. If you refresh or leave, the plan disappears.</p></div>
        </div>
      )}
      <div className="form-stack">
        <section className="planner-section">
          <div className="form-section-heading">
            <span>01</span><div><h3>Your available cash</h3>
        ...
        <section className="planner-section">
          <div className="form-section-heading"><span>02</span><div><h3>Your paycheck and spending</h3>
          <p>This keeps the plan inside your normal cash flow.</p></div></div>
          <div className="form-grid">
        ...
        <section className="planner-section">
          <div className="form-section-heading"><span>03</span><div><h3>Choose your pace</h3>
          <p>Start simple or compare more offers.</p></div></div>
        ...
        <section className="planner-section">
          <div className="form-section-heading"><span>04</span><div><h3>Estimated tax rate</h3>
          <p>Bonuses and interest can be taxable. Use “I don’t know” by leaving this off.</p></div></div>
      ...
      <div className="form-actions">
        {guestMode && <Link className="text-link back-link" href="/"><ArrowLeft size={16} /> Back home</Link>}
        <button className="button primary" type="submit" disabled={saving}>{saving ? "Building…" : <>{guestMode ? "See my practice plan" : "Build my plan"} <ArrowRight size={18} /></>}</button>
      </div>
    </form>
  );
}
